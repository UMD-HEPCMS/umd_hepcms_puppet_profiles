...
#
class profile::ovirt::guest (
  $manage_qemu_agent = true,
  $qemu_agent_package_ensure = 'present',
  $qemu_agent_package_name = $profile::ovirt::params::qemu_agent_package_name,
  $qemu_agent_service_ensure = 'running',
  $qemu_agent_service_enable = true,
  $qemu_agent_service_name = $profile::ovirt::params::qemu_agent_service_name,
  $ovirt_agent_package_ensure = 'present',
  $ovirt_agent_package_name = $profile::ovirt::params::ovirt_agent_package_name,
  $ovirt_agent_service_ensure = 'running',
  $ovirt_agent_service_enable = true,
  $ovirt_agent_service_name = $profile::ovirt::params::ovirt_agent_service_name,
) inherits profile::ovirt::params {
